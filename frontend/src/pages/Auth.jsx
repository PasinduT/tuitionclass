...
export default function Auth() {
  ...
  return <div className="grid min-h-screen bg-paper lg:grid-cols-[1.05fr_.95fr]">
    <section className="noise relative hidden overflow-hidden bg-ink p-14 text-white lg:flex lg:flex-col">
      <div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-leaf-600"><Sparkles size={22}/></div><div><p className="font-display text-2xl font-extrabold">pahas</p><p className="text-[10px] font-bold uppercase tracking-[.2em] text-white/40">Learn with clarity</p></div></div>
    ...
    <section className="flex items-center justify-center p-5 sm:p-10"><div className="w-full max-w-md"><div className="mb-10 flex items-center gap-3 lg:hidden"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-leaf-600 text-white"><Sparkles size={20}/></div><p className="font-display text-2xl font-extrabold">pahas</p></div><Badge>{mode === 'signin' ? 'WELCOME BACK' : 'START YOUR JOURNEY'}</Badge><h2 className="mt-4 font-display text-3xl font-extrabold tracking-tight">{mode === 'signin' ? 'Ready for another win?' : 'Create your learning space'}</h2><p className="mt-2 text-sm text-slate-500">{mode === 'signin' ? 'Sign in and pick up where you left off.' : 'A few details, then your practice plan is ready.'}</p>
